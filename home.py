...
language = st.sidebar.select_slider(label='Select language', options=['Q#', 'Qiskit'])
...
fig_bits = go.Figure()
fig_bits.add_trace(go.Histogram(x=bits))
fig_bits.update_xaxes(title='Bit', tickfont_size=15)
fig_bits.update_yaxes(title='Count', tickfont_size=15)
fig_bits.update_layout(width=1500)
...
fig_bytes = go.Figure()
fig_bytes.add_trace(go.Histogram(x=bytes))
fig_bytes.update_xaxes(title='Byte', tickfont_size=15)
fig_bytes.update_yaxes(title='Count', tickfont_size=15)
fig_bytes.update_layout(width=1500)
